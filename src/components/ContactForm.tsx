...
export default function DrawerDialogDemo() {
  ...
  let message = "Shoot me a message and I will get back to you as soon as possible!"
  if (state.succeeded) {
    title = "Thank you 😊";
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent>
        ...
        <ProfileForm className="tw-px-4" formState={state} handleFormSubmit={handleSubmit} />
        <DrawerFooter className="tw-pt-2">
          <DrawerClose asChild>
            <Button variant="outline">{ state.succeeded ? 'Dismiss' : 'Cancel' }</Button>
          </DrawerClose>
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
...
function ProfileForm({ className, formState, handleFormSubmit }) {
  ...
  return (
    <form onSubmit={handleFormSubmit} className={cn("tw-grid tw-items-start tw-gap-4", className)}>
      <div className="tw-grid tw-gap-2">
        <Label htmlFor="name">Name</Label>
        <Input type="text" id="name" placeholder="Someone I can't wait to meet" required/>
        <ValidationError 
        prefix="Name" 
        field="name"
      ...
      <Button type="submit" variant="outline" disabled={formState.submitting}>Submit</Button>
    </form>
  )
}
